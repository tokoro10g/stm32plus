...
namespace stm32plus {
   ...
   * @tparam TTimerNumber The number of the timer 1..4
   */
  ...
  inline void TimerInterruptFeatureEnabler<1>::enable(uint16_t interruptMask,uint8_t priority,uint8_t subPriority) {
    ...
    if((interruptMask & TIM_IT_Update)!=0) {
      _forceLinkage=&TIM1_UP_TIM16_IRQHandler;
      Nvic::configureIrq(TIM1_UP_TIM16_IRQn,ENABLE,priority,subPriority);
    }

    if((interruptMask & TIM_IT_Break)!=0) {
      _forceLinkage=&TIM1_BRK_TIM15_IRQHandler;
      Nvic::configureIrq(TIM1_BRK_TIM15_IRQn,ENABLE,priority,subPriority);
    }

    if((interruptMask & (TIM_IT_COM | TIM_IT_Trigger))!=0) {
      _forceLinkage=&TIM1_TRG_COM_TIM17_IRQHandler;
      Nvic::configureIrq(TIM1_TRG_COM_TIM17_IRQn,ENABLE,priority,subPriority);
    }
